...
@ManagedBean
@ViewScoped
public class RotaTesteMb implements Serializable {
	...
	public void terminarCriacaoOuAlteracao() {
		try {
			...
			pr.setDescricao("pontorota1");
			pr.setId(12212l);
			RequestContext.getCurrentInstance().addCallbackParam("rota", pr);
	...
	public void onPointSelect(PointSelectEvent event) {
		if (pontoReferencia != null || rota.getPontos().size() == 0) {
			...
			RequestContext.getCurrentInstance().execute("alert('testando')");
			RequestContext.getCurrentInstance().addCallbackParam("lat", pontoRota.getLat());
			RequestContext.getCurrentInstance().addCallbackParam("lng", pontoRota.getLng());
		}
	}
